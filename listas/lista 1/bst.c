...
int sucessor(arvore a, int valor){
	arvore temp = a;
	int encontrou = 0;

	while(!encontrou && temp != NULL){
		if(temp->valor == valor){
			encontrou = 1;
			break;
		}else{
	...
	if(encontrou){
		...
		if(temp->dir != NULL){
			...
			//caso 2 - Sem filhos
			//o sucessor de X será o ancestral mais baixo de X cujo filho da esquerda também seja um ancestral de X
			return ancestralMaisBaixo(a,valor);
			}
		}	
	else{
		return -1;
...
int ancestralMaisBaixo(arvore a, int valor){
	if(valor == a->valor){
		...
		int resultEsq = ancestralMaisBaixo(a->esq, valor);
		if(resultEsq == -1){
			return a->valor;
		}else {
			return resultEsq;
		 ...
		 return ancestralMaisBaixo(a->dir, valor);
	}
}
